; (function game(){
    ...
    const pressedKeyActionMap = {
        ...
        Space(timestamp) {
            ...
            addFireBall(wizardCoordinates.x + wizard.offsetWidth, wizardCoordinates.y + wizard.offsetHeight * 0.4);
    ...
    function processFireBalls() {
        scene.fireBalls.forEach(fireball => {
            const newX = (config.speed * config.fireBallMultiplier) + utils.pxToNumber(fireball.style.left);
    ...
    const procesClouds = processGameElementFactory(addCloud, 200, 'lastCloudSpawnTimestamp', 'clouds', 'cloudSpawnInterval');
    const processBug = processGameElementFactory(addBug, 60, 'lastBugSpawnTimestamp', 'bugs', 'bugSpawnInterval', bugElementProcessor);
